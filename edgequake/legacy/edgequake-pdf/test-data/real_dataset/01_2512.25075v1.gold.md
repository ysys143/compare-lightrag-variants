...
t=80t=80reversemotiont=10t=0t=20t=40t=60Source VideoSynthesized Videot=70t=80t=60t=40t=20t=0bullet-timeat t=60Source VideoSynthesized Videot=10t=0t=20t=40t=60t=60t=60t=60t=60t=60t=60slow motionx0.5Source VideoSynthesized Videot=10t=0t=20t=40t=60t=5t=0t=10t=20t=30t=40Frame Number80 你的段落⽂字 Two ballet dancers in blue and white costumes performing a graceful routine on a dark stage with a red velvet curtain and dramatic spotlights.A couple in 18th-century Baroque attire dancing on a theater stage in front of a live symphony orchestra; warm theatrical lighting.A couple performing a retro swing dance on a Parisian bridge; woman in a red polka-dot dress, classic Haussmann buildings in the background.Camera Trajactory ControlAnimation Time Control0t=80
...
In this work, we introduce SpaceTimePilot,
...
CameraCameraCameraCamera Control Models4D Multi-View ModelsSpaceTimePilotTimeobserved inputsynthesized views or starting frames of videosframe progressionTimeTimetemporal-control mechanism that enables finer-grained ma-
...
inspiration from
[2], which introduces explicit camera
ReCamMaster
...
Instead,
(visual evaluations are provided in Supp. Mat.).
...
frame space, (cid:101)e = Conv1D2(Conv1D1(e)). Finally, we add
these time features to the camera features and video tokens

4

1234567811223344556677881234555567855555555123456543211234567812345678(f) Freeze (a) Standard Forward(b) Reverse Playback(e) ZigZag Motion(d) Global Slow Motion x 0.5(c) Motion-to-FreezePivot P, Where [N/2] < P < NFreezing P for N/2 steps, Where 0 < P < NPivot PRepeat P for whole video, Where 0 < P < NSource VideoTarget VideoStandard Forwardframe index(a) Camera Control TI2V (c) Temporal Wrapping (ours)dynamic vidoes(2) Comparison of different training scheme for time control(b) Joint Dataset Trainingstatic scene videosdynamic vidoesframe indexframe indexanimation time t w.r.t. source animation time t w.r.t. source animation time t w.r.t. source (1) Temporal Warping Augmentationembeddings, updating Eq. (1) as follows:
...
f =1 and a target video Vtrg = {I f
...
trg =
{I τ (f )
f =1. The source animation timestamps are uni-
...
continuous camera traj.animation timefull space-time grid rendering ..................................................................source video............Table 2. Quantitative comparison across temporal controls (Direction (forward, backward motion), Speed (slow modes), Bullet Time). We
...
ReCamM+preshuffled†
17.13 14.84 14.61 15.52 0.6623 0.6050 0.5965 0.6213 0.3930 0.4793 0.4863 0.4529
ReCamM+jointdata
18.32 17.57 17.69 17.86 0.7322 0.7220 0.7209 0.7250 0.2972 0.3158 0.3089 0.3073
...
In the bullet-time example, condi-
...
bullet-timeat t=40bullet-timeat t=40slow motion40-80reversemotionslow motion40-80reversemotiont=40t=0t=80t=20t=40Source VideoSynthesized VideoSource VideoSynthesized VideoSource VideoSynthesized VideoSource VideoSynthesized VideoSource VideoSynthesized VideoSource VideoSynthesized VideoFrame Number800Frame Number800Table 4. Camera accuracy and first-frame estimation. For camera control,
...
t=0t=20t=40t=80t=80t=60t=40t=0t=80t=60t=40t=0t=80t=60t=40t=0Src VideoOursReCamMasterTrajectoryCrafterframe 0frame 20frame 40frame 80Camera: pan left. Time: freeze at t = 40.Camera: tilt-down. Time: freeze at t = 40.MLP Compressor1D-Conv CompressorUniform Resamplingframe 0frame 40frame 80MLP Compressor1D-Conv CompressorUniform Resamplingt=40t=40t=40frame 0frame 40frame 80t=40t=40t=40frame 0frame 40frame 80t=40t=40t=40frame 0frame 40frame 80t=40t=40t=40frame 0frame 40frame 80t=40t=40t=40frame 0frame 40frame 80t=40t=40t=40reverse playback. TrajectoryCrafter, in contrast, is confused
...
Abhijit Kundu, Dmitry Lagun,
Issam Laradji, Hsueh-
Ti
(Derek) Liu, Henning Meyer, Yishu Miao, Derek
...
abling camera control for text-to-video generation. In ICLR, 2025. 8
...
radiance field rendering. ACM Trans. Graph., 42(4):139–1, 2023. 2
...
sistent 4d generation via video diffusion models. In NeurIPS, 2024. 2

[22] Jinwei Lin. Dynamic NeRF: A review.
...
learning-based 3d vision. 2024. 2, 5
...
thesis. In ECCV, pages 99–106. ACM New York, NY, USA, 2021. 2
...
long-term spatial memory. arXiv preprint arXiv:2506.05284, 2025. 3
...
erative view synthesis with diffusion models. arXiv preprint, 2025. 3
...
treme monocular dynamic novel view synthesis. ECCV, 2024. 2, 3, 4, 5
...
IJCV, pages 1–20,
with cascaded latent diffusion models. 2024. 2
...
linear tracking motions, and
include rotational orbits,
...
fffffZsrcZtrgAdd noisePatchifySpace-Time DiTZ’trg3D-VAEDecoder3D-VAEencoderVt’ /t/tCameraembeddingTimeEmbeddingSelf-AttentionProjectorCross-AttentionFeed-forward NNcamcamcamcamcamcamcamcamtimetimetimetimetimetimetimetime(patchified Ztrg)(patchified Zsrc)Figure 9. Overview of the multi-turn autoregressive inference scheme. The model first generates an 81-frame segment conditioned
...
SpaceTimePilotdual src videoSource video1 (81 frames)camera traj 1(wrt src video 1)temporal traj 1 (wrt src video 1)SpaceTimePilotdual src videoOutput video 1(81 frames)Output video 2(81 frames)Video StitchingFinal Output. (81 \* n frames)camera traj 2(wrt src video 1)temporal traj 2 (wrt src video 1)SpaceTimePilotdual src videocamera traj 2(wrt src video 1)temporal traj 2 (wrt src video 1)Copy of Source video1...scene datasets naturally support bullet-time effects, they do
...
bullet-timeat t=40reversemotionbounce40-80-40slow motion40-80Frame 0Frame 40Frame 80......Frame 0Frame 40Frame 80......bounce40-80-40bounce40-80-40slow motion40-80bounce40-80-40Figure 14. Ablation study. (Top) Using [1, 2] as default datasets, we compare the influence of adding static-scene datasets [18, 53] vs. just
...
Src VideoDefault+RE10k+MCDefault w/ TWAblation 1: Temporal warping (TW) vs. Joint dataset training on bullet-time effect (t=40)Src Video1D-Conv1DConv+Cam×Timew/o FreezingTemproalWarping1DConv+Cam×Time wFreezingTemporalWarpingAblation 2: Varied temporal warping configuration on bullet-time effect (t=40) Figure 15. Ablation study. (Top) We verify the efficacy of the proposed Cam×Time dataset. Considering [1, 2] as default datasets,
...
Src VideoDefault w/ TWDefault w/ TW ＋ Cam×TimeAblation 3: Dataset ablation: Cam×Time vs. static-scene datasets on bullet-time effect (t=40) Default w/ TW ＋ RE10k ＋ MC Src VideoMLPUniformSampling1D-Conv1D-Conv w/Cam×TimeAblation 4: Different time embedding schemes on bullet-time effect (t=40)RoPE(f’)
